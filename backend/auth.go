...
func (c *Conn) auth(authData []byte, plugin string) ([]byte, error) {
	switch plugin {
	case "caching_sha2_password":
		authResp := scrambleSHA256Password(authData, c.password)
		return authResp, nil
	...
	case "mysql_old_password":
		...
		authResp := append(scrambleOldPassword(authData[:8], c.password), 0)
		return authResp, nil
	...
	case "mysql_native_password":
		...
		authResp := scramblePassword(authData[:20], c.password)
		return authResp, nil
	...
	case "sha256_password":
		...
		// encrypted password
		// enc, err := encryptPassword(c.password, authData, pubKey)
		// return enc, err

	default:
		// mc.cfg.Logger.Print("unknown auth plugin:", plugin)
		return nil, mysql.ErrUnknownPlugin
	}
}
	...
	case "caching_sha2_password":
		...
		case 1:
			...
			case 4: // auth full
				...
				// 	// write cleartext auth packet
				// 	err = c.writeAuthSwitchPacket(append([]byte(c.cfg.Passwd), 0))
				// 	if err != nil {
				// 		return err
				// 	}
				// } else {
				...
				if data[0] != mysql.AUTH_MORE_HEADER {
